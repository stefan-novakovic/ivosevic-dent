...
const Hero = () => {
   ...
   useEffect(() => {
      if (isImageLoaded) {
         setIsVisible(true); // Show text after image is loaded
      }
   }, [isImageLoaded]);
   ...
   return (
      <div
         ...
         className="portrait:h-[calc((100vh-67px)/1.2625)] landscape:h-auto md:portrait:h-[calc(100vh-78px)] md:landscape:h-[calc(100vh-78px)] md:min-h-[413px] select-none bg-light-bg/20 dark:bg-dark-bg/5 transition-colors duration-200 overflow-hidden relative bg-[77%_33%] md:bg-[66.5%_33%] bg-no-repeat bg-cover"
      >
         {/* Hero Image */}
         <img
            src={hero.heroSrc}
            ...
            onLoad={() => setIsImageLoaded(true)} // Trigger animation after image loads
         />
         <h1
